                            ...
                            <div class="box">
                                <div class="box-header">
                                    <div style="padding:10px;">
                                        <button class="btn btn-primary btn-sm hidden" data-toggle="modal" data-target="#addModal"><i class="fa fa-user-plus" aria-hidden="true"></i> Add Clearance</button>  
                                ...
                                <form method="post">
                                    ...
                                    <div id="approved" class="tab-pane active in">
                                        <table id="table_clearance1" class="table table-bordered table-striped">
                                            <thead>
                                                <tr>
                                                    <?php if(!isset($_SESSION['staff'])) { ?>
                                            ...
                                            <tbody>
                                                ...
                                                if(!isset($_SESSION['staff']))
                                                {
                                                    ...
                                                    $squery = mysqli_query($con, "SELECT *,CONCAT(r.lname, ', ' ,r.fname, ' ' ,r.mname) as residentname,p.id as pid FROM tblclearance p left join tblresident r on r.id = p.residentid  where status = 'Approved' AND `statusArchive` = 1") or die('Error: ' . mysqli_error($con));
                                                    while($row = mysqli_fetch_array($squery))
                                                    {
                                                        ...
                                                        <tr>
                                                            ...
                                                            <a target="_blank" href="clearance_form.php?resident='.$row['residentid'].'&clearance='.$row['clearanceNo'].'&val='.base64_encode($row['clearanceNo'].'|'.$row['residentname'].'|'.$row['dateRecorded']).' class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Generate</a></td>
                                                        </tr>
                                                        ';
                                ...
                                <form method="post">                                        
                                        <table id="table_clearance1" class="table table-bordered table-striped">
                                            ...
                                            <tbody>
                                                <?php
                                                $squery = mysqli_query($con, "SELECT *,CONCAT(r.lname, ', ' ,r.fname, ' ' ,r.mname) as residentname,p.id as pid FROM tblclearance p left join tblresident r on r.id = p.residentid  where status = 'New' AND `statusArchive` = 1") or die('Error: ' . mysqli_error($con));
                                                while($row = mysqli_fetch_array($squery))
                                                {
                                                    echo '
                                                    <tr>
                                        ...
                                        <table id="table_clearance2" class="table table-bordered table-striped">
                                            <thead>
                                                <tr>
                                                    <?php  // if(!isset($_SESSION['staff'])) { ?>
                                                    <!-- <th style="width: 20px !important;"><input type="checkbox" name="chk_delete[]" class="cbxMain" onchange="checkMain(this)"/></th> -->
                                            ...
                                            <tbody>
                                                ...
                                                if(!isset($_SESSION['staff']))
                                                {
                                                    $squery = mysqli_query($con, "SELECT *,CONCAT(r.lname, ', ' ,r.fname, ' ' ,r.mname) as residentname,p.id as pid FROM tblclearance p left join tblresident r on r.id = p.residentid where status = 'Disapproved' AND `statusArchive` = 1") or die('Error: ' . mysqli_error($con));
                                                    while($row = mysqli_fetch_array($squery))
                                                    {
                                                        echo '
                                                        <tr>
                                ...
                                <form method="post">
                                 <div class="tab-content">
                                    ...
                                    <table id="table_clearance1" class="table table-bordered table-striped">
                                        ...
                                        <tbody>
                                            <?php
                                            $squery = mysqli_query($con, "SELECT * FROM tblclearance p left join tblresident r on r.id = p.residentid where r.id = ".$_SESSION['userid']." AND status = 'New' AND `statusArchive` = 1") or die('Error: ' . mysqli_error($con));
                                    ...
                                    <table id="table_clearance1" class="table table-bordered table-striped">
                                        ...
                                        <tbody>
                                            <?php
                                            $squery = mysqli_query($con, "SELECT * FROM tblclearance p left join tblresident r on r.id = p.residentid where r.id = ".$_SESSION['userid']." AND status = 'Approved' AND `statusArchive` = 1") or die('Error: ' . mysqli_error($con));
                                            if(mysqli_num_rows($squery) > 0)
                                            {
                                                while($row = mysqli_fetch_array($squery))
                                                {
                                    ...
                                    <table id="table_clearance1" class="table table-bordered table-striped">
                                        ...
                                        <tbody>
                                            <?php
                                            $squery = mysqli_query($con, "SELECT * FROM tblclearance p left join tblresident r on r.id = p.residentid where r.id = ".$_SESSION['userid']." AND status = 'Disapproved' AND `statusArchive` = 1") or die('Error: ' . mysqli_error($con));
                                            if(mysqli_num_rows($squery) > 0)
                                            {
                                                while($row = mysqli_fetch_array($squery))
                                                {
...
<script type="text/javascript">
    ...
    ?>
        $(function() {
            $("#table_clearance1").dataTable({
               "aoColumnDefs": [ { "bSortable": false, "aTargets": [ 0,7 ] } ],"aaSorting": []
            });
            $("#table_clearance2").dataTable({
