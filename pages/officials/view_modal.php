  ...
  <div class="modal-dialog">
    <div class="modal-content">
        <div class="modal-header">
            ...
            <h4 class="modal-title">View Photo for officials'.$row['officials'].'</h4>
        </div>
        ...
        <div class="row">';
            $p = mysqli_query($con,"SELECT * from tblofficialphoto where officialId = '".$row['id']."' ");
            while($row1 = mysqli_fetch_array($p)){
                echo '<div class="col-md-4">
                    <input type="checkbox" name="chk_deletephoto[]" class="chk_deletephoto" value="'.$row1['id'].'" />
                    <image src="image/'.basename($row1['filename']).'" style="width:170px;height:170px;"/>
                </div>';
            }
